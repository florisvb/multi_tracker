...
class Trajectory:
    def __init__(self, hdf5data, objid, start_time=0):
        ...
        self.length = len(self.objid_indices)
    ...
    def __frames__(self):
        ...
        return f
    def __date__(self):
        lt = time.localtime(self.time_epoch[0])
...
class Dataset:
    ...
    def framestamp_to_timestamp(self, frame):
        indices = np.where(self.hdf5data['header.frame_id']==frame)[0][0]
        t = self.hdf5data[indices]['header.stamp.secs'] + self.hdf5data[indices]['header.stamp.nsecs']*1e-9 
        return t
        
    def timestamp_to_framestamp(self, t):
        allt = self.hdf5data['header.stamp.secs'] + self.hdf5data['header.stamp.nsecs']*1e-9 
        index = np.argmin( np.abs(allt-t) )
        return self.hdf5data[index]['header.frame_id']
