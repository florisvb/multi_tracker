...
def calc_actual_covariance_for_trajectory_given_q(trajec, q=1, r=10, no_data_after_nth_frame=30):
    '''
    ...
    for frame in range(0,no_data_after_nth_frame):
        ...
        actual_position = np.array([trajec.measured_position[frame, 0], trajec.measured_position[frame,1]] )
    ...
    for frame in range(no_data_after_nth_frame,trajec.length):
...
def calc_actual_covariance_for_all_trajecs_given_q(data, keys, q=1, r=10, no_data_after_nth_frame=30):
    '''
    ...
    for key in keys:
        trajec = data[key]
        p_, k_ = find_covariance_Q_for_trajectory(trajec, q, r, no_data_after_nth_frame)
        p.append(p_)
        k.append(k_)
