...
def get_heatmap(pd, binsx, binsy):
    x = pd['position_x'].values
    y = pd['position_y'].values
    h, xedges, yedges = np.histogram2d(x,y,bins=[binsx,binsy])
    return h
    
def plot_heatmap(pd, binsx, binsy, ax=None, vmin=0, vmax=None, logcolorscale=False):   
    if ax is None:
        fig = plt.figure()
        ax = fig.add_subplot(111)
        
    heatmap = get_heatmap(pd, binsx, binsy)
    if logcolorscale:
        heatmap = np.log(heatmap)
    if vmax is None:
        vmax = 0.1*np.max(heatmap)
    ax.imshow(heatmap.T, cmap=plt.get_cmap('hot'), vmin=vmin, vmax=vmax)
    
def plot_trajectories(pd, binsx, binsy, backgroundimage=None, ax=None):
    if ax is None:
        fig = plt.figure()
        ax = fig.add_subplot(111)
