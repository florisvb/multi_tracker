...
def get_candidate_keys(pd, key, framerange=5):
    dataset = rhfp.Dataset(pd)
    last_frame = dataset.trajec(key).frames[-1]
    candidate_keys = data_slicing.get_keys_in_framerange(pd, [last_frame-framerange, last_frame+framerange]).tolist()
...
def check_distance_on_candidate_keys(pd, key, candidate_keys, max_distance=10):
    ...
    for candidate in candidate_keys:
        # don't accept candidates that full contain all the frames of the key
        if dataset.trajec(candidate).frames[0] < dataset.trajec(key).frames[0] and dataset.trajec(candidate).frames[-1] > dataset.trajec(key).frames[-1]:
            e = np.inf
...
def get_link_chain(pd, key, framerange=5, max_distance=14):
    chain = [key]
    candidate_keys = [key]
...
def get_complete_chain(pd, framerange=5, max_distance=14):
    chains = []
